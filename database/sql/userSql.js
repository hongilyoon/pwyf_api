...
exports.getUserJsonLastUpdateDate = "SELECT u.seq AS seq, u.id AS id, MAX(j.mod_date) AS modDate FROM user u LEFT OUTER JOIN json j ON u.seq = j.user_seq WHERE u.id = ? GROUP BY u.seq";